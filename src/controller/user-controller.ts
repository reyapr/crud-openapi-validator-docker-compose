...
export class UserController implements IUserController {
  ...
  async findAll(_: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      ...
      const userResponse: IUserResponse[] = result.map((user) =>
        UserController.constructUserResponse(user)
      )
  ...
  async findById(
    ...
    try {
      ...
      const userResponse: IUserResponse =
        UserController.constructUserResponse(result)
  ...
  async update(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      const { id } = req.params
      const { name, email, role, password } = req.body
      const affected = await this.userService.update(id as string, {
        name,
        email,
        role,
        password
